#' Linear Regression Model using Rcpp for computation
#'
#' This function fits a simple linear regression model to the provided data,
#' but uses C++ via Rcpp to compute the coefficients efficiently.
#' It returns coefficients, fitted values, residuals,
...
#'
#' @return A list summarizing the fitted model results.
#'
#' @examples
#' data(mtcars)
#' x <- as.matrix(mtcars[, c("wt", "hp", "cyl")])
#' y <- mtcars$mpg
#' results <- lm_625_withcpp(x, y)
#' print(results)
#'
#' @export
#'
lm_625_withcpp <- function(x, y) {
  if (!is.matrix(x)) {
  ...
  # Use Rcpp implementation here
  beta_hat <- rcpp_lm(X, Y)
